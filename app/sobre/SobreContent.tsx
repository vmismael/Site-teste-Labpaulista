...
export default function SobreContent() {
  ...
  const estruturaRef = useRef<HTMLElement>(null);
  ...
  useEffect(() => {
    ...
    const historiaEl = historiaRef.current;
    if (historiaEl) {
      const text = historiaEl.querySelector<HTMLElement>(".historia-text");
      const img = historiaEl.querySelector<HTMLElement>(".historia-img");
      if (text && img) {
        gsap.set(text, { opacity: 0, x: -30 });
        gsap.set(img, { opacity: 0, x: 30 });
        const obs = new IntersectionObserver(
          ([entry]) => {
            if (entry.isIntersecting) {
              gsap.to(text, { opacity: 1, x: 0, duration: 0.7, ease: "power2.out" });
              gsap.to(img, { opacity: 1, x: 0, duration: 0.7, ease: "power2.out", delay: 0.1 });
              obs.disconnect();
            }
          },
          { threshold: 0.15 }
        );
        obs.observe(historiaEl);
        observers.push(obs);
      }
    }

    const missaoEl = missaoRef.current;
    if (missaoEl) {
      const cards = missaoEl.querySelectorAll<HTMLElement>(".missao-card");
      if (cards.length) {
        ...
        obs.observe(missaoEl);
        observers.push(obs);
      }
    }

    const estruturaEl = estruturaRef.current;
    if (estruturaEl) {
      const img = estruturaEl.querySelector<HTMLElement>(".estrutura-img");
      const text = estruturaEl.querySelector<HTMLElement>(".estrutura-text");
      if (img && text) {
        gsap.set(img, { opacity: 0, x: -30 });
        gsap.set(text, { opacity: 0, x: 30 });
        const obs = new IntersectionObserver(
          ([entry]) => {
            if (entry.isIntersecting) {
              gsap.to(img, { opacity: 1, x: 0, duration: 0.7, ease: "power2.out" });
        ...
        obs.observe(estruturaEl);
        observers.push(obs);
      }
    }

    const acoesEl = acoesRef.current;
    if (acoesEl) {
      const cards = acoesEl.querySelectorAll<HTMLElement>(".acao-card");
      if (cards.length) {
        gsap.set(cards, { opacity: 0, y: 24 });
        const obs = new IntersectionObserver(
          ([entry]) => {
  ...
  return (
    <>
      ...
      {/* ── Nossa história ── */}
      <section ref={historiaRef} className="bg-white py-24" aria-labelledby="historia-heading">
        <div className="container-content">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="historia-img relative h-[500px] rounded-[4px] overflow-hidden">
              <Image
      ...
      {/* ── Missão e Visão ── */}
      <section ref={missaoRef} className="relative bg-[#0a0a0a] overflow-hidden" aria-labelledby="missao-heading">
        <div className="absolute inset-0">
          <Image
        ...
        <div className="relative container-content py-24">
          <div className="max-w-4xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="missao-card border-l-2 border-[#c8102e] pl-8 py-2">
      ...
      {/* ── Estrutura ── */}
      <section ref={estruturaRef} className="bg-white py-24" aria-labelledby="estrutura-heading">
        <div className="container-content">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="estrutura-img relative h-[460px] rounded-[4px] overflow-hidden order-last lg:order-first">
              <Image
                src="/lab-cafe.webp"
        ...
        <div className="container-content">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              ...
              <div
                key={titulo}
                className="acao-card bg-white border border-[rgba(0,0,0,0.07)] rounded-[4px] p-7 hover:border-[rgba(0,0,0,0.14)] transition-colors"
              >
