...
export default function ComplianceContent() {
  ...
  useEffect(() => {
    ...
    const pilaresEl = pilaresRef.current;
    if (pilaresEl) {
      const cards = pilaresEl.querySelectorAll<HTMLElement>(".pilar-card");
      if (cards.length) {
        ...
        <div className="container-content">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {PILARES.map(({ titulo, descricao }) => (
              <div
                key={titulo}
                className="pilar-card bg-white border border-[rgba(0,0,0,0.07)] rounded-[4px] p-7 hover:border-[rgba(0,0,0,0.14)] transition-colors"
              >
